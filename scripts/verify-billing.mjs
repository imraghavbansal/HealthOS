...
async function main() {
  const email = `raag-verify-billing-${Math.random().toString(36).slice(2, 8)}@mailinator.com`;
  ...
  const fakeActivate = {
    ...
    payload: { subscription: { entity: { id: "sub_fake", current_end: Math.floor(Date.now() / 1000) + 2592000, notes: { raag_user_id: userId, raag_plan_key: "pro" } } } },
  };
  ...
  const fakeCancel = {
    ...
    payload: { subscription: { entity: { id: "sub_fake", notes: { raag_user_id: userId, raag_plan_key: "pro" } } } },
  };
