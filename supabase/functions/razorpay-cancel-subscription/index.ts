...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: billing } = await supabase.from("billing").select("razorpay_subscription_id").eq("user_id", userData.user.id).single();
    if (!billing?.razorpay_subscription_id) return json({ error: "No active subscription found" }, 404);

    const keyId = Deno.env.get("RAZORPAY_KEY_ID")!;
    const keySecret = Deno.env.get("RAZORPAY_KEY_SECRET")!;
    const res = await fetch(`https://api.razorpay.com/v1/subscriptions/${billing.razorpay_subscription_id}/cancel`, {
      method: "POST",
