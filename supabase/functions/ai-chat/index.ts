// Atlas Health — ai-chat
//
// Grounds "Ask Atlas" in the user's own structured data — not a blank-slate
...
const RED_FLAG_PATTERNS: { pattern: RegExp; label: string }[] = [
  { pattern: /chest (pain|pressure|tightness)|crushing (chest|pain)/i, label: "possible cardiac emergency" },
  { pattern: /can'?t breathe|difficulty breathing|severe shortness of breath|gasping for air/i, label: "breathing emergency" },
  { pattern: /face (is )?droop|slurred speech|sudden numbness|sudden weakness|can'?t (move|feel) (my|one side)/i, label: "possible stroke" },
  { pattern: /want to (die|kill myself)|suicidal|suicide|end my life|self.?harm/i, label: "mental health crisis" },
  { pattern: /throat (is )?closing|can'?t swallow|severe allergic reaction|anaphylax/i, label: "possible anaphylaxis" },
  { pattern: /won'?t stop bleeding|uncontrolled bleeding|bleeding heavily/i, label: "severe bleeding" },
...
const SYSTEM_PROMPT = `You are Atlas, a personal health assistant. You help the user understand their own health history — you never diagnose, prescribe, or claim certainty a clinician would need to confirm.
...
async function buildContext(supabase: ReturnType<typeof createClient>, subjectId: string): Promise<string> {
  const [labs, meds, vitals, symptoms, conditions, family, docs] = await Promise.all([
    supabase.from("lab_markers").select("id, name, value, unit, range_low, range_high, collected_at").eq("subject_id", subjectId).order("collected_at", { ascending: false }).limit(40),
    supabase.from("medications").select("id, name, dose, schedule, type").eq("subject_id", subjectId).eq("active", true),
    supabase.from("vitals").select("id, kind, value, secondary, unit, recorded_at").eq("subject_id", subjectId).order("recorded_at", { ascending: false }).limit(20),
    supabase.from("symptoms").select("id, label, severity, body_area, started_at").eq("subject_id", subjectId).order("started_at", { ascending: false }).limit(10),
    supabase.from("conditions").select("id, name, status, diagnosed_at").eq("subject_id", subjectId),
    supabase.from("family_history_entries").select("id, relation, conditions").eq("subject_id", subjectId),
    supabase.from("source_documents").select("id, title, document_type, document_date, summary").eq("subject_id", subjectId).order("document_date", { ascending: false }).limit(20),
  ]);
  ...
  if (labs.data?.length) {
    sections.push(
      "LAB RESULTS (most recent first):\n" +
        labs.data.map((l) => `- ${l.name}: ${l.value} ${l.unit} (range ${l.range_low ?? "?"}–${l.range_high ?? "?"}) on ${formatDate(l.collected_at)} ${tag(l.id, "lab_markers")}`).join("\n"),
    );
  }
  if (meds.data?.length) {
    sections.push("ACTIVE MEDICATIONS:\n" + meds.data.map((m) => `- ${m.name}${m.dose ? ` ${m.dose}` : ""}${m.schedule ? `, ${m.schedule}` : ""} (${m.type}) ${tag(m.id, "medications")}`).join("\n"));
  }
  if (vitals.data?.length) {
    sections.push(
      "RECENT VITALS:\n" +
        vitals.data.map((v) => `- ${v.kind}: ${v.value}${v.secondary ? `/${v.secondary}` : ""} ${v.unit} on ${formatDate(v.recorded_at)} ${tag(v.id, "vitals")}`).join("\n"),
    );
  }
  if (symptoms.data?.length) {
    sections.push("RECENT SYMPTOMS:\n" + symptoms.data.map((s) => `- ${s.label} (severity ${s.severity}/10, ${s.body_area ?? "unspecified area"}) started ${formatDate(s.started_at)} ${tag(s.id, "symptoms")}`).join("\n"));
  }
  if (conditions.data?.length) {
    sections.push("CONDITIONS:\n" + conditions.data.map((c) => `- ${c.name} (${c.status}${c.diagnosed_at ? `, diagnosed ${formatDate(c.diagnosed_at)}` : ""}) ${tag(c.id, "conditions")}`).join("\n"));
  }
  if (family.data?.length) {
    sections.push("FAMILY HISTORY:\n" + family.data.map((f) => `- ${f.relation}: ${(f.conditions ?? []).join(", ") || "none reported"} ${tag(f.id, "family_history_entries")}`).join("\n"));
  }
  if (docs.data?.length) {
    sections.push(
      "UPLOADED DOCUMENTS:\n" +
        docs.data.map((d) => `- "${d.title}" (${d.document_type}, ${formatDate(d.document_date)})${d.summary ? `: ${d.summary}` : " — not yet parsed"} ${tag(d.id, "source_documents")}`).join("\n"),
    );
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: subject, error: subjectErr } = await supabase.from("health_subjects").select("id, name").eq("id", subjectId).single();
    ...
    if (emergency) {
      const { data: saved, error: saveErr } = await supabase
        .from("chat_messages")
        .insert({ subject_id: subjectId, user_id: callerId, role: "assistant", content: emergency, citations: [] })
        .select()
        .single();
      ...
      const stream = new ReadableStream({
        start(controller) {
          ...
          controller.enqueue(sseLine({ type: "done", id: saved.id, citations: [], createdAt: saved.created_at }));
          controller.close();
        },
      });
      return new Response(stream, { headers: { ...CORS_HEADERS, "content-type": "application/x-ndjson" } });
    }
    ...
    const { data: profile } = await supabase.from("profiles").select("plan").eq("id", callerId).single();
    if (profile?.plan === "free") {
      ...
      if ((count ?? 0) > FREE_MONTHLY_LIMIT) {
        ...
        const { data: saved, error: saveErr } = await supabase
          .from("chat_messages")
          .insert({ subject_id: subjectId, user_id: callerId, role: "assistant", content: limitMessage, citations: [] })
          .select()
          .single();
        if (saveErr) throw saveErr;
        const stream = new ReadableStream({
    ...
    await supabase.from("audit_log").insert({ actor_user_id: callerId, subject_id: subjectId, action: "ai_context_read", resource: "health_subjects" });

    const [context, recentHistory] = await Promise.all([
      buildContext(supabase, subjectId),
      supabase.from("chat_messages").select("role, content").eq("subject_id", subjectId).order("created_at", { ascending: false }).limit(10),
    ]);

    const history = (recentHistory.data ?? []).reverse();
    const messages = history.length ? history.map((h) => ({ role: h.role as "user" | "assistant", content: h.content })) : [{ role: "user" as const, content }];
        ...
        try {
          for await (const event of anthropicStream) {
            if (event.type === "content_block_delta" && event.delta.type === "text_delta") {
              ...
              if (safeLen > flushed) {
                controller.enqueue(sseLine({ type: "delta", text: fullText.slice(flushed, safeLen) }));
                flushed = safeLen;
              }
            }
          }
          ...
          const { data: saved, error: saveErr } = await sbForClose
            .from("chat_messages")
            .insert({ subject_id: subjectIdForClose, user_id: callerIdForClose, role: "assistant", content: replyContent, citations })
            .select()
            .single();
          if (saveErr) throw saveErr;

          controller.enqueue(sseLine({ type: "done", id: saved.id, citations: saved.citations, createdAt: saved.created_at }));
        } catch (err) {
          console.error(err);
          controller.enqueue(sseLine({ type: "error", error: err instanceof Error ? err.message : String(err) }));
        } finally {
          controller.close();
        }
      },
