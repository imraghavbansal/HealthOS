...
import { useFamilyHistory, useRisks } from "@/lib/queries";
import type { FamilyMember, RiskFactor } from "@/lib/types";
...
function groupRelation(relation: string): "Paternal" | "Maternal" | "Siblings" | "Other" {
  ...
  if (r.includes("father") || r.includes("paternal") || r.includes("uncle") && r.includes("dad")) return "Paternal";
...
function Family() {
  ...
  return (
    <AppShell title="Family History & Risk" subtitle="Understand inherited risk. Act early.">
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <p className="text-[11px] text-muted-foreground text-center">
            Risk levels are directional estimates based on your inputs, not diagnoses. Informational only — not medical advice.
          </p>
        </div>
      </div>
    </AppShell>
...
function FamilyTree({ members }: { members: FamilyMember[] }) {
  const groups: Record<string, FamilyMember[]> = { Paternal: [], Maternal: [], Siblings: [], Other: [] };
    ...
    <div className="space-y-5">
      {(["Paternal", "Maternal", "Siblings", "Other"] as const).map((groupName) =>
        groups[groupName].length > 0 ? (
          <div key={groupName}>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">{groupName}</div>
                ...
                <StaggerItem key={f.relation}>
                  <div className="rounded-2xl border border-border/60 p-4">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {f.conditions.map((c) => (
                        <span key={c} className="text-[10px] px-2 py-0.5 rounded-full bg-accent/70 border border-border">
                          {c}
                        </span>
                      ))}
                    </div>
      ...
      <Card className="rounded-3xl border-border/60">
        <CardContent className="p-5">
          <div className="flex items-center justify-between">
            ...
            <Badge className={`rounded-full text-[10px] ${LEVEL_TONE[risk.level] ?? ""}`}>{risk.level} risk</Badge>
          </div>
          ...
          <Collapsible open={open} onOpenChange={setOpen} className="mt-3">
            <CollapsibleTrigger asChild>
              <Button variant="ghost" size="sm" className="rounded-full h-7 px-2 text-xs text-primary">
                What you can do
