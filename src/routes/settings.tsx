      ...
      <div className="grid lg:grid-cols-[240px_1fr] gap-6">
        <nav className="space-y-1 h-fit">
          {["Account", "Billing", "Privacy", "Security", "Data", "Notifications"].map((s, i) => (
            <a key={s} href={`#${s}`} className={`block rounded-xl px-3 py-2 text-sm ${i === 0 ? "bg-primary/10 text-foreground" : "text-muted-foreground hover:bg-accent/50"}`}>{s}</a>
          ))}
        </nav>
        <div className="space-y-6">
          ...
          <p className="text-[11px] text-muted-foreground text-center">
            Raag is informational and not a substitute for professional medical advice, diagnosis, or treatment.
          </p>
        </div>
      </div>
    </AppShell>
...
function AccountSection() {
  ...
  const [form, setForm] = useState<Pick<UserProfile, "name" | "timezone" | "units">>({ name: "", timezone: "", units: "metric" });
  const [dirty, setDirty] = useState(false);

  useEffect(() => {
    if (profileQ.data && !dirty) {
      setForm({ name: profileQ.data.name, timezone: profileQ.data.timezone, units: profileQ.data.units });
    }
  }, [profileQ.data, dirty]);

  return (
    <Section id="Account" title="Account" desc="Your basic profile.">
      <AsyncBoundary query={profileQ} skeleton={<div className="h-24 animate-pulse rounded-xl bg-muted" />}>
        {(profile) => (
          <>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Field label="Timezone">
                <Input value={form.timezone} onChange={(e) => { setForm((f) => ({ ...f, timezone: e.target.value })); setDirty(true); }} />
              </Field>
              <Field label="Units">
                <select
                  className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs"
                  value={form.units}
                  onChange={(e) => { setForm((f) => ({ ...f, units: e.target.value as "metric" | "imperial" })); setDirty(true); }}
                >
...
const PLAN_LABELS: Record<string, string> = { free: "Free", pro: "Pro", family: "Family", clinic: "Clinic" };
...
function BillingSection() {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      toast.success("Cancellation requested — you'll keep access until the end of your current billing period.");
    } catch (err) {
  ...
  return (
    <Section id="Billing" title="Billing" desc="Your plan and payment." icon={CreditCard}>
      <AsyncBoundary query={profileQ} skeleton={<div className="h-16 animate-pulse rounded-xl bg-muted" />}>
        {(profile) => (
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium">Current plan: {PLAN_LABELS[profile.plan] ?? profile.plan}</div>
              <p className="text-xs text-muted-foreground mt-0.5">
                {profile.plan === "free" ? (
                  <>Visit <Link to="/pricing" className="underline">Pricing</Link> to upgrade.</>
                ) : (
            ...
            {profile.plan !== "free" && (
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="outline" className="rounded-full" disabled={cancelling}>Cancel plan</Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Cancel your {PLAN_LABELS[profile.plan]} plan?</AlertDialogTitle>
                    <AlertDialogDescription>
                      You'll keep {PLAN_LABELS[profile.plan]} access until the end of your current billing period,
                      then move to Free. You can resubscribe anytime.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
...
function PrivacySection() {
  ...
  return (
    <Section id="Privacy" title="Privacy & consent" desc="Fine-grained control over how your data is used." icon={Shield}>
      <AsyncBoundary query={consentQ} skeleton={<div className="h-24 animate-pulse rounded-xl bg-muted" />}>
        {(consent) => (
          <>
            <Toggle
...
function SecuritySection() {
  ...
  return (
    <Section id="Security" title="Security" desc="Keep your Raag locked down." icon={Lock}>
      ...
      <Button variant="outline" className="rounded-full" onClick={changePassword} disabled={changing || !next}>
      ...
      <div className="opacity-60">
        <Toggle label="Two-factor authentication (TOTP)" desc="Coming soon." checked={false} onChange={() => {}} disabled />
        <Toggle label="Biometric unlock" desc="Coming with the mobile app." checked={false} onChange={() => {}} disabled />
      </div>
    </Section>
  );
}
...
function DataSection() {
  ...
  return (
    <Section id="Data" title="Your data" desc="Export or permanently delete everything." icon={Download}>
      <div className="grid md:grid-cols-2 gap-3">
        <div className="rounded-2xl border border-border/60 p-4">
          <div className="font-medium text-sm">Export</div>
          <p className="text-xs text-muted-foreground mt-1">Download everything Raag has stored about you as JSON.</p>
        ...
        <div className="rounded-2xl border border-destructive/40 bg-destructive/5 p-4">
          <div className="font-medium text-sm flex items-center gap-1.5 text-destructive"><AlertTriangle className="h-4 w-4" /> Delete account</div>
          <p className="text-xs text-muted-foreground mt-1">Permanently deletes every record. Cannot be undone.</p>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" className="mt-3 rounded-full"><Trash2 className="mr-1.5 h-4 w-4" /> Delete everything</Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Delete your account?</AlertDialogTitle>
                <AlertDialogDescription>
                  This permanently deletes your profile, records, labs, medications, and every other piece of data Raag
...
function NotificationsSection() {
  ...
  return (
    <Section id="Notifications" title="Notifications" desc="Only the reminders that matter.">
      <AsyncBoundary query={prefsQ} skeleton={<div className="h-24 animate-pulse rounded-xl bg-muted" />}>
        {(prefs) => (
          <>
            ...
            <Toggle label="Big changes in trends" desc="Only when Raag thinks it's worth a look." checked={prefs.trendAlerts} onChange={(v) => updatePrefs.mutate({ trendAlerts: v })} />
...
function Section({ id, title, desc, icon: Icon, children }: { id: string; title: string; desc: string; icon?: React.ComponentType<{ className?: string }>; children: React.ReactNode }) {
  return (
    <Card id={id} className="rounded-3xl border-border/60">
      <CardContent className="p-6 space-y-4">
        <div>
          <div className="flex items-center gap-2 font-semibold">{Icon && <Icon className="h-4 w-4 text-primary" />} {title}</div>
