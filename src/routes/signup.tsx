  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (needsEmailConfirmation) {
        ...
        toast.success(`Welcome to Atlas Health, ${form.name.trim().split(" ")[0]} 👋`);
        nav({ to: "/onboarding" });
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-md mx-auto px-6 py-16">
        ...
        <div className="rounded-3xl glass p-8 md:p-10">
          {awaitingConfirmation ? (
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              <h2 className="font-display text-3xl">Create your Atlas.</h2>
              ...
              <Button type="submit" disabled={submitting} className="w-full rounded-full gradient-primary text-white border-0 shadow-soft">
                {submitting ? "Creating your Atlas…" : "Continue"} <ArrowRight className="ml-1 h-4 w-4" />
              </Button>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          By continuing you accept the informational nature of Atlas — not medical advice.
        </p>
      </div>
    </div>
  );
